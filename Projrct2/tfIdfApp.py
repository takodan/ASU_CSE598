...
def answer():
    ...
    if not not_filtered_texts:
        ...
        for not_filtered_text in not_filtered_texts:
            ...
            filtered_texts = filtered_texts + [response.data.decode("utf-8")]
            response.close()
...
def get_first_two_links(url):
    ...
    for link in links[:2]:
        if "http" in link['href']:
            ...
            result = result + [(root_url+link['href'])]
            # else:
            #     result = result + [(url+link['href'])]

    return result # [link['href'] for link in links[:2]]
